...
export default function WeddingPageView({
	...
	return (
		<div
			className={`min-h-screen ${theme.bg} ${theme.text} ${fontClass} selection:bg-neutral-200`}
		>
			...
			{/* ── Hero Section ────────────────────────────────────────────── */}
			<section
				className={`relative h-screen flex items-center justify-center overflow-hidden ${inviteeName && !envelopeOpen ? "pt-16" : ""}`}
			>
				<motion.div
				...
				>
					<img
						src={
							wedding.heroImage ||
							"https://picsum.photos/seed/wedding/1920/1080"
						}
						alt="Wedding Hero"
						...
						<div className="space-y-4">
							<div className="flex flex-col items-center">
								...
								<p className="text-xl font-medium">
									{wedding.date
										? format(new Date(wedding.date), "MMMM do, yyyy")
										: "TBD"}
								</p>
							</div>
							<div className="flex flex-col items-center">
						...
						<div className="space-y-4">
							<div className="flex flex-col items-center">
								...
								<p className="opacity-80">
									{wedding.venueAddress || "Venue Address"}
								</p>
			...
			<section className="py-24 px-4 max-w-2xl mx-auto">
				...
				>
					...
					{!isPreview ? (
						<a
							href={`https://wa.me/6281237042471?text=${encodeURIComponent(waMessage)}`}
							target="_blank"
							rel="noreferrer"
						...
						<div
							className={`inline-flex items-center justify-center gap-3 px-10 py-4 rounded-full font-semibold text-lg opacity-50 cursor-not-allowed ${theme.button}`}
						>
							<MessageCircle className="w-5 h-5" />
							Konfirmasi via WhatsApp
						</div>
