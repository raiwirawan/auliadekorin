...
import {
  Heart, Plus, Globe, EyeOff, ExternalLink, Copy, LogOut,
  Loader2, Calendar, Sparkles, LayoutDashboard, AlertCircle
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const handleTogglePublish = async (id: number) => {
    setTogglingId(id);
    try {
      const res = await fetch(`/api/weddings/${id}/publish`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      setWeddings(prev =>
        prev.map(w => w.id === id ? { ...w, is_published: data.is_published } : w)
      );
    } catch (err: any) {
      alert(err.message);
    } finally {
      ...
      <header className="relative z-10 border-b border-white/10 bg-white/5 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2">
            <div className="w-9 h-9 bg-gradient-to-br from-rose-400 to-purple-600 rounded-xl flex items-center justify-center shadow-lg">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link
              ...
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-rose-500 to-purple-600 text-white text-sm font-bold rounded-xl hover:opacity-90 transition-opacity shadow-lg shadow-rose-500/20"
            >
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 py-12">
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-rose-400 text-sm font-semibold uppercase tracking-wider">My Wedding Pages</span>
          </div>
          <h1 className="text-4xl font-bold mb-2">Dashboard</h1>
          <p className="text-white/50">Manage your wedding pages, control visibility, and share links.</p>
        </motion.div>
        ...
        {isLoading ? (
          ...
          >
            ...
            <p className="text-white/40 mb-8 max-w-sm">You haven't created any wedding pages yet. Start crafting a beautiful invitation for your special day.</p>
            <Link
              to="/create"
              className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-rose-500 to-purple-600 text-white font-bold rounded-xl hover:opacity-90 transition-opacity shadow-lg shadow-rose-500/20"
            >
              ...
              {weddings.map((w, i) => (
                ...
                >
                  ...
                  <h3 className="text-lg font-bold mb-1">
                    {w.bride_name} <span className="text-rose-400">&</span> {w.groom_name}
                  </h3>

                  {/* Slug */}
                  <p className="text-white/40 text-xs font-mono mb-1 truncate">/w/{w.slug}</p>

                  {/* Date */}
                  {w.date && (
                    <div className="flex items-center gap-1.5 text-white/40 text-xs mb-5">
                      <Calendar className="w-3 h-3" />
                      {new Date(w.date).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' })}
                    </div>
                  )}
